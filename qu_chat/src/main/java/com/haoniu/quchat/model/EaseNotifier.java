...
import com.haoniu.quchat.base.EaseUI.EaseSettingsProvider;
...
public class EaseNotifier {
    ...
    protected void sendNotification(EMMessage message, boolean isForeground, boolean numIncrease) {
        ...
        if (message.getStringAttribute(Constant.MSGTYPE, "").equals(Constant.RABEND) || message.getStringAttribute(Constant.MSGTYPE, "").equals(Constant.RAB) ) {
            return;
        }
        try {
    ...
    public void vibrateAndPlayTone(EMMessage message) {
        if (message != null) {
            if (EaseCommonUtils.isSilentMessage(message) || message.getFrom().equals("系统管理员")) {
                return;
            }
        }
